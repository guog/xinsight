...
`
  ...
  it("无效输入时抛出有意义的错误", async () => {
    await expect(parseOpenApiSpec("not valid json or yaml{{{")).rejects.toThrow(
      /无法解析/,
    )
    await expect(
      parseOpenApiSpec({ random: "object" } as Record<string, unknown>),
    ).rejects.toThrow(/不是有效的 OpenAPI/)
  })
