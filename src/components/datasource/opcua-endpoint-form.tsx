...
export default function OpcuaEndpointForm({ endpoint, onChange, onRemove }: Props) {
  const update = (partial: Partial<OpcuaEndpoint>) =>
    onChange({ ...endpoint, ...partial } as OpcuaEndpoint)

  const addNodeId = () => update({ nodeIds: [...endpoint.nodeIds, ""] })
  const removeNodeId = (i: number) =>
    update({ nodeIds: endpoint.nodeIds.filter((_, idx) => idx !== i) })
  const updateNodeId = (i: number, val: string) =>
    update({ nodeIds: endpoint.nodeIds.map((n, idx) => (idx === i ? val : n)) })
      ...
      <div className="grid gap-3 sm:grid-cols-3">
        ...
        <div>
          ...
          <select
            ...
            onChange={(e) => update({ action: e.target.value as OpcuaEndpoint["action"] })}
          >
            {actions.map((a) => (
          ...
          {endpoint.nodeIds.map((nodeId, i) => (
            <div key={i} className="flex gap-2">
              <input
                className={inputClass}
                value={nodeId}
                onChange={(e) => updateNodeId(i, e.target.value)}
                placeholder="ns=2;s=Temperature"
              />
              <button
                type="button"
                onClick={() => removeNodeId(i)}
