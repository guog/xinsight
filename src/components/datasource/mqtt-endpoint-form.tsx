...
export default function MqttEndpointForm({ endpoint, onChange, onRemove }: Props) {
  const update = (partial: Partial<MqttEndpoint>) => onChange({ ...endpoint, ...partial } as MqttEndpoint)
  ...
  return (
    <div className="border border-border rounded-lg p-4 space-y-3">
      ...
      <div>
        ...
        <input className={inputClass} value={endpoint.topic} onChange={(e) => update({ topic: e.target.value })} placeholder="sensors/temperature/#" />
      </div>

      <div className="grid gap-3 sm:grid-cols-3">
        <div>
          <label className={labelClass}>Direction</label>
          <select className={inputClass} value={endpoint.direction} onChange={(e) => update({ direction: e.target.value as MqttEndpoint["direction"] })}>
            {directions.map((d) => <option key={d} value={d}>{d}</option>)}
          </select>
        </div>
        <div>
          <label className={labelClass}>QoS</label>
          <select className={inputClass} value={endpoint.qos} onChange={(e) => update({ qos: Number(e.target.value) as 0 | 1 | 2 })}>
        ...
        <div>
          ...
          <select className={inputClass} value={endpoint.payloadFormat} onChange={(e) => update({ payloadFormat: e.target.value as MqttEndpoint["payloadFormat"] })}>
            {payloadFormats.map((f) => <option key={f} value={f}>{f}</option>)}
          </select>
        </div>
      </div>
